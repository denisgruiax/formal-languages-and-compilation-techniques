...
/*Variabile globale*/
Token *tokens;
char *pCrtCh, *text, *text2, text3[100];
int line = 1;
...
void readFile(char *path)
{
    ...
    if ((fileSource = fopen(path, "r")) == NULL)
    {
...
Token *createToken(int code)
{
    ...
    {
        token->text = malloc(strlen(text2) * sizeof(char));
        strcpy(token->text, text2);
    }
    else if (code == CT_CHAR)
    {
        if (text2[0] == '\\')
        {
            ...
            token->integer = text2[0];
    }
    else if (code == CT_INT)
    {
        if ((text2[0] == '0') && (text2[1] == 'x'))
        {
            char *junk;
            token->integer = strtol(text2 + 2, &junk, 16);
        }
        else
            token->integer = atol(text2);
    }
    else if (code == CT_REAL)
        token->real = atof(text2);
...
int getNextToken()
{
    ...
    do
    {
        ...
        case 31:
            ...
            {
                text2 = createString(pStartCh, pCrtCh);
                tokens = addToken(ID);
            }
